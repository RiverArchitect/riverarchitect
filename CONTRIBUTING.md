...
* Before creating the Pull Request, run the test suite and build the documentation a last
  time:

    ```shell
    pytest
    python -m sphinx -b html docs docs/_build/html   # must stay at zero warnings
    ```
...
* In GitHub, send a pull request to `RiverArchitect/riverarchitect:main`. This will trigger the check of the
