...
Two modules, and the feature definitions they share. The features themselves - what each one is, and where its default thresholds come from - are catalogued in {doc}`features`.
...
Output rasters are named after the feature id: `lf_<fid>.tif` for a lifespan map, `ds_<fid>.tif` for a design map.
...
Which features use which, and why, is in {doc}`features`.

### Dimensionless bed shear stress (`taux`)

River Architect keeps the historical name `taux` for the **Shields parameter**. It is not a dimensional stress and it is not the $x$-component of a stress tensor. For the reference grain fraction $D_{84}$,
...
It contains **one** base-10 logarithm. It is a depth-integrated resistance law derived from a logarithmic wall profile; it does not require two logarithmic layers in the water column, and it does not assert that the instantaneous velocity profile is logarithmic from the grain crests to the free surface.
...
This switch assumes turbulent, hydraulically rough gravel or cobble flow. Check the roughness Reynolds number $k_s^+=u_*k_s/\nu$: if viscosity still affects the resistance, use a smooth or transitionally rough resistance law or, preferably, bed shear exported by the hydraulic model. The switch also cannot separate grain stress from bar, step, bank, vegetation or large-wood form drag.
```
...
Use a measured $D_{84}$ raster when possible. The fallback $D_{84}\approx2.2D_{50}$ was the median relation in the Rickenmann-Recking field data and is only justified when the input represents $D_{50}$. A file called `dmean.tif` is not automatically $D_{50}$; record what statistic it contains. Compare $\theta_{84}$ with a critical Shields value defined for the same reference fraction, or apply an explicit hiding/exposure correction.
```

Primary sources: [Keulegan (1938)](https://nvlpubs.nist.gov/nistpubs/jres/21/jresv21n6p707_A1b.pdf), [Einstein (1950)](https://books.google.com/books?id=ah_n36gVE2gC), and [Rickenmann and Recking (2011)](https://doi.org/10.1029/2010WR009793).

#### Where it lives, and what it writes

{func}`riverarchitect.shear.calculate_taux` is the single implementation; both {meth}`riverarchitect.lifespan.LifespanDesign.shields_stress` and {meth}`riverarchitect.recruitment.RecruitmentPotential.shields_stress` call it, so the two modules cannot drift apart. It is pure numpy and takes plain arrays, so it can be used directly on any aligned depth, velocity and $D_{84}$ rasters.
...
`tb` holds $u_*^2$ rather than the dimensional stress $\rho_w u_*^2$ its name suggests, because that is the quantity `ts` is formed from and the density cancels. Documented here rather than renamed, so existing projects keep reading.

They are written in two places, and both write the same numbers:

* **into the condition folder**, by {func}`riverarchitect.preprocessing.bed_shear_stress`, reachable as **Get Started ▸ dimensionless bed shear stress (taux)**. This is the standalone stress map for a whole condition;
* **into the output folder**, by Lifespan Design and Riparian Recruitment, for the discharges each actually used.

Nothing *reads* them back: both analyses recompute the stress from depth, velocity and grain size, so a stale or hand-edited `ts<Q>.tif` cannot quietly change a result.

Read `regime<Q>.tif` before trusting a stress map. On the bundled sample reach about 95 % of wet cells are regime 1, which is where a single logarithmic resistance law would be outside its range of validity.
...
The values in `FEATURES` and in a threshold workbook are U.S. customary numbers applied as they are, against data in the condition's own unit system. This looks like a missing unit conversion and is not one.
```

Two-argument {func}`riverarchitect.raster.con` is used throughout, so a criterion's false branch is NoData and never zero; zero there is what creates wetted-area artefacts.
...
**Ties are kept rather than broken.** A cell where two features both reach the maximum appears in both layers, which is why the reported shares add to more than 100 per cent. That is deliberate: it tells the planner the choice is theirs.

The result feeds {doc}`morphology` - the `best_<feature>.tif` masks are what Terraforming reads to know where a feature is planned.
