...
<a href="LifespanDesign">LifespanDesign</a> layouts:
 ...
 - `layout_ds_bio` for [bioengineering features](River-design-features.md#bioeng)
 - `layout_ds_Dcr` for [grain mobility maps](River-design-features.md#rocks) and [sediment replenishment / gravel augmentation](River-design-features.md#gravel)
 - `layout_ds_Dw` for [wood](River-design-features.md#elj) diameter mobility maps
 - `layout_ds_FS_D15` for [fine sediment](River-design-features.md#finesed)'s mobile *D<sub>15</sub>* sizes
 - `layout_ds_FS_D85` for [fine sediment](River-design-features.md#finesed)'s mobile *D<sub>85</sub>* sizes
 - `layout_ds_SeS0` for ratios between terrain and (hypothetic) energy slopes ([side channels](River-design-features.md#sidechnl))
 - `layout_ds_sideca` for [side cavity](River-design-features.md#sidecav) application site mapping
 - `layout_ds` for [widen](River-design-features.md#berms) application site mapping
 
<a href="MaxLifespan">Max Lifespan</a> layouts:

 - `layout_mlf_bioeng` for best lifespan maps of the (other) [bioengineering feature group](River-design-features.md#featoverview)
 - `layout_mlf_maintenance` for best lifespan maps of the [maintenance/longitudinal connectivity feature group](River-design-features.md#featoverview)
 - `layout_mlf_plants` for best lifespan maps of the [vegetation plantings feature group](River-design-features.md#plants)
 - `layout_mlf_terraforming` for best lifespan maps of the  [terraforming feature group](River-design-features.md#featoverview)
...
The global map extents are determined by the input rasters to map. The same is true for raster calculations. The application scheme of *River Architect* starts with the usage of the <a href="LifespanDesign">LifespanDesign</a> module. Here, the calculation can be limited to the `background.tif` raster. Thus, for changing map extents, adapt the extent of input rasters for the <a href="LifespanDesign">LifespanDesign</a> and <a href="MaxLifespan">Max Lifespan</a> modules. For the <a href="ModifyTerrain">Modify Terrain</a> module adapt the `dem.tif` raster in the input/ [condition](Signposts.md#conditions) folder, which determines the map extents. Moreover, when applying <a href="ModifyTerrain">Modify Terrain</a>, do not use another [condition](Signposts.md#conditions) folder for the modified DEM for volume calculation, because it may interfere, for example, with the `dem_detrend.tif` raster. Preferably use a copy or extract the modified condition's `dem.tif` to `ModifyTerrain/Input/DEM/CONDITION/`.
...
Moreover, each module has an own mapping definition file to enable reach-wise or multiple map creation within one `PDF`. The particular map extent definitions are:
 ...
 - <a href="ModifyTerrain">Modify Terrain</a>: `RiverArchitect/ModifyTerrain/.templates/computation_extents.xlsx` ([see reach definitions](ModifyTerrain.md#input-set-reaches))
