...
***
**Please refer to the [installation guide](Installation.md#started) for [first-time installation](Installation.md#started), [update](Installation.md#update_ra), and [launch](Installation.md#launch_ra) assistance.**
...
*River Architect* invites to analyses and modifications of the longevity and ecological quality of riverscapes. Different planning bases ("conditions") can be easily created using an introductory module called **[GetStarted](Signposts.md#getstarted)**. **Lifespan**, **Morphology (Terraforming)** and **Ecohydraulic** assessments can then be created based on the *Conditions*, including the creation of project plans and cost tables with a **Project Maker** module.

**[Lifespan](LifespanDesign.md)** maps indicate the expected longevity of restoration features as a function of terrain change, morphological characteristics, and 2D hydrodynamic modeling results. **Design maps** are a side product of [lifespan and design mapping](LifespanDesign.md) and indicate required feature dimensions for stability, such as the minimum required size of angular boulders to avoid their mobilization during floods (more information in [Schwindt et al.2019][11]). **Best lifespan maps** result from the comparison of lifespan and design maps of multiple restoration features and assign features with the highest longevity to each pixel of a raster. Thus, the **[Max Lifespan](MaxLifespan.md)** module assess optimum features as a function of highest lifespans among comparable feature groups such as terraforming or vegetation planting species.

**Morphology (Terraforming)** includes routines to **[Modify Terrain](ModifyTerrain.md)** for river restoration purposes. Currently, two terrain modification algorithms are implemented: (1) Threshold value-based terrain modifications in terms of [grading](River-design-features.md#grading) or [widening / broaden rivers](River-design-features.md#berms) for riparian forest establishment; and (2) [River Builder](RiverBuilder.md) for the creation of synthetic river valley. A **[Volume Assessment](VolumeAssessment.md)** module can compare an original (pre-project or pre-terraforming application) and a modified DEM (\"with implementation\" or post-feature application) to determine required earth movement (terraforming volumes) works.

**Ecohydraulics** assessments include the evaluation of the ecohydraulic state and connectivity of riverscapes. The **[Habitat Area](SHArC.md)** (Seasonal Habitat Area Calculator) module applies user-defined flows (discharges) for the spatial evaluation of the habitat suitability index (*HSI*) in terms of Seasonal Habitat Area (SHArea). The hydraulic habitat suitability results from 2D hydrodynamic numerical model outputs of flow depth and velocity. Also, a  \"cover\" option can be used to assess ecohydraulic effects of cobble, boulder, vegetation, and streamwood. The **[Stranding Risk](StrandingRisk.md)** module provides insights into the connection of wetted areas on floodplains and how these may be improved to enhance the survivorship of fry / juvenile fish. The **[Riparian Seedling Recruitment](RSR.md)** module provides an assessment of the potential seedling recruitment area across a site given a daily mean flow record and can be used to evaluate design topographies effectiveness at increasing natural seedling recruitment. 

The **[Project Maker](ProjectMaker.md)** module creates preliminary construction plans and evaluates the costs for gain in usable habitat for target fish species and lifestages. A unit cost workbook provides relevant costs and the gain in usable habitat area results from the *SHArC* module.

A set of **[Tools](Tools.md)** provides *Python* console scripts that are under development and will be implemented in future versions of the *River Architect* GUI.
...
2.  Determine relevant discharges for 2D hydrodynamic modeling:
    ...
    -   At least three flood discharges against which potential [restoration features](River-design-features.md) have to withstand (determine lifespan intersects).
3.  Run a 2D hydrodynamic model (steady) with all determined discharges to generate hydraulic snap-shots of the river.
4.  Create a [Condition](Signposts.md#conditions) using the [GetStarted](Signposts.md#getstarted) module. The *Condition* should include *GeoTIFFs* of the initial (existing or pre-project) river state:

    -   Detrended digital elevation model (DEM);

    -   Flow depth and velocity for multiple discharges Rasters from 2D hydrodynamic modeling (see [Signposts](Signposts.md#conditions));
    ...
    -   Datasets that can be used to assess design feature stability, such as [side channel design criteria](River-design-features.md#sidechnl);
...
5.  Apply the [*LifespanDesign*](LifespanDesign.md) module to [framework (terraforming) features](River-design-features.md#featoverview).
6.  Lifespan and Design maps, as well as expert assessment, serve for the identification of relevant [framework (terraforming) features](River-design-features.md#featoverview).
7.  Iterative terraforming (if relevant):

    -   Use the [*ModifyTerrain*](ModifyTerrain.md) module for creating synthetic river valley with [River Builder](RiverBuilder.md); or apply threshold value-based terrain [grading](River-design-features.md#grading) or [broadening of the river bed](River-design-features.md#berms). *Please note that both routines require post-processing with computer-aided design (translation into real-world coordinates and/or edge smoothing).*
...
8.  Apply the [*LifespanDesign*](LifespanDesign.md) module to [vegetation plantings](River-design-features.md#plants) and [(other) bioengineering features](River-design-features.md#bioeng) based on the terraformed DEM (or the original / initial DEM if no terraforming applies).
9.  Use the [*MaxLifespan*](MaxLifespan.md) module to identify best performing (highest lifespan) [vegetation plantings](River-design-features.md#plants) and [(other) bioengineering features](River-design-features.md#bioeng).
10.  If the soils are too coarse (i.e., the capillarity is not high enough to enable plant root growth), apply the connectivity feature ["incorporate fine sediment in soils"](River-design-features.md#finesed).
11. If gravel augmentation methods are applicable: Consecutively apply the [*LifespanDesign*](LifespanDesign.md) and [*MaxLifespan*](MaxLifespan.md) modules to [connectivity features](River-design-features.md#featoverview) to foster self-sustaining, artificially created ecomorphological patterns within the terraforming process.<br/>
    If gravel is added in-stream, re-run the numerical model for the assessment of [gravel stability](River-design-features.md#rocks) with the [*LifespanDesign*](LifespanDesign.md) module and the combined habitat suitability with the [*SHArC*][6] module to compare the [**S**easonal **H**abitat **Area** (**SHArea**)](SHArC.md#herunSHArea) before and after enhancement of [Physical Habitats for target fish species (lifestages)](SHArC.md#hefish).
...
The working principles of the [*LifespanDesign*](LifespanDesign.md), [*MaxLifespan*](MaxLifespan.md), [*ModifyTerrain*](ModifyTerrain.md), *<a href="VolumeAssessment">VolumeAssessment</a>*, [*SHArC*][6], [*StrandingRisk*](StrandingRisk.md), [*RiparianSeedlingRecruitment*](RSR.md) and [*ProjectMaker*][7] modules are explained on their own Wiki pages. The differentiation between [terraforming (framework)](River-design-features.md#featoverview), [vegetation plantings and other bioengineering](River-design-features.md#featoverview), and [connectivity features](River-design-features.md#featoverview) is described within the [LifespanDesign Wiki](River-design-features.md). The <a href="Installation">Installation</a> Wiki pages describe the propper installation, file organization and environment of *River Architect*.
