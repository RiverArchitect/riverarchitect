...
- **Maximum data size (*River Architect*'s Lifespan & Design mapping hangs without prompting any message)**: *River Architect*'s data processing competence depends on the available Random-Access Memory (RAM). As a general rule, the maximum amount of data should be smaller than the available RAM. The size of processed data can be approximated as:<br/>RAM >= *Single Raster size* · *Number of features* · *Number of parameters per feature* · *Number of discharges*<br/>The data size limit applies to lifespan mapping only, where *River Architect* loads nearby all available data in the `01_Conditions/CONDITION/` folder for each selected feature to run [`arcpy.sa.CellStatistics`](https://pro.arcgis.com/en/pro-app/tool-reference/spatial-analyst/cell-statistics.htm). To avoid issues, consider the following rules of thumb:
	 ...
	 * Limit the number of discharges to less than 10 (set relevant discharges defined in rows 4, 11, and 12 of [*input files*](Signposts.md#inpfile) to less than 10); manual modification of automatically generated input files may be required (rows 4, 11, and 12 of `01_Conditions/CONDITION/input_definitions.inp`).
...
Most errors occur when the wrong python interpreter is used or when Rasters or layouts have bad formats or when the information stated in the [input files](Signposts.md#inpfile) is erroneous.
...
- **`ERROR 999998: Unexpected Error.`** This is an operating system error and it can indicate different error conditions (i.e., the real reasons may have various error sources). Some of the most probable causes are:
    ...
    - *Remedy:*
        + Make sure to use the correct *Python* environment ([see installation guide](Installation.md#install-and-get-started))
...
- **`ERROR: .cache folder will be removed by package controls.`**
 ...
 - **`ERROR: Area calculation failed.`**
    - *Cause:*   Raised by `calculate_sha(self)` of [*SHArC*][6]'s `CHSI()` class in `SHArC/cHSI.py` when it could not calculate the usable habitat area (see [SHArea Methods](SHArC.md#herunaua)).
    - *Remedy:*
        + Ensure that the SHArea threshold has a meaningful value between 0.0 and 1.0 ([SHArC Intro](SHArC.md)).
        ...
        + Review the input settings according to the [SHArC Quick GUIde](SHArC.md#hegui).
 ...
 - **`ERROR: Bad assignment of x/y values in the coordinate input file.`**
    ...
    - *Remedy:*  Ensure that the coordinate definitions in `mapping.inp` (`LifespanDesign/.templates/` or `MaxLifespan/.templates/`) correspond to the definitions in [*LifespanDesign* Output maps](LifespanDesign.md#outmaps).
 ...
 - **`ERROR: Bad call of map centre coordinates. Creating squared-x layouts.`**
    ...
    - *Remedy:*
        + [*LifespanDesign*][3]: Ensure that the file `mapping.inp` exists in the directory `LifespanDesign/.templates/` corresponding to the definitions in [*LifespanDesign* Output maps](LifespanDesign.md#outmaps).
        + [*MaxLifespan*][4]: Ensure that the file `mapping.inp` exists in the directory `MaxLifespan/.templates/` corresponding to the definitions in [*LifespanDesign* Output maps](LifespanDesign.md#outmaps).
        + General: Replace `mapping.inp` with the original file and re-apply modifications strictly following [*LifespanDesign* Output maps](LifespanDesign.md#outmaps).
 ...
 - **`ERROR: Boundary shapefile in arcpy.PolygonToRaster[...].`**
    ...
    - *Remedy:*  Verify that a the selected boundary shapefile ([SHArC computation boundaries](SHArC.md#hebound)) has a valid rectangle and an `Id` field value of `1` for that rectangle.
...
- **`ERROR: Could not access Fish.xlsx (...).`**
 ...
 - **`ERROR: Could not add cover HSI.`**
    ...
    - *Remedy:*  Manually verify cover HSI Rasters in `SHArC/HSI/` and recompile cover HSI Rasters if needed (see [CoverHSI](SHArC.md#hemakecovhsi)).
 ...
 - **`ERROR: Could not append PDF page XX to map assembly.`**
    ...
    - *Remedy:*
	    + <a href="LifespanDesign">LifespanDesign</a> / <a href="MaxLifespan">Max Lifespan</a>: Ensure that the definitions in [`LifespanDesign/.templates/mapping.inp`](Signposts.md#inpfile) are correct.
	    + <a href="VolumeAssessment">VolumeAssessment</a>: Verify the definitions in the [reach coordinates workbook](RiverReaches.md) and also refer to error message `ERROR: Could not create new project`.
 ...
 - **`ERROR: Could not calculate CellStatistics (Raster comparison).`**
    ...
    - *Remedy:*  Manually open the input Rasters and ensure that they comply with the requirements stated in the [Conditions / Input Rasters section](Signposts.md#conditions).
...
- **`ERROR: Could not create new project (...)`**
 ...
 - **`ERROR: Could not create Raster of the project area.`**
    ...
    - *Remedy:*
        + Ensure that the project was correctly delineated ([Project Area Polygon preparation](ProjectMaker.md#pminp2)).
        + If the Error message occurred within the terrain stabilization process, either restart the Project creation while properly implementing the [Project Area Polygon preparation](ProjectMaker.md#pminp2) OR manually create a project area Raster (save as `ProjectMaker/ProjectName_vXX/Geodata/Rasters/ProjectArea.tif`, where in-values=`Int(1)` and out-values=`NoData`).
...
- **`ERROR: Could not create sub-condition folder: ... .`**
 ...
 - **`ERROR: Could not find any worksheet.`**
    ...
    - *Remedy:*
        + Ensure the correct usage of `.site_packages/templates/Fish.xlsx` ([SHArC Fish section](SHArC.md#hefish)).
        + Ensure the correct adaptation of `ProjectMaker/.../Project_assessment_vii.xlsx` ([ProjectMaker's Cost quantity workbook](ProjectMaker.md#pmcq)).
 ...
 - **`ERROR: Could not find sheet "extents" in computation_extents.xlsx.`**
    ...
    - *Remedy:*  Ensure the correct setup of `ModifyTerrain/.templates/computation_extents.xlsx` ([reach preparation within the *ModifyTerrain* module](RiverReaches.md)).
 ...
 - **`ERROR: Could not find the cover input geofile [...]`**
    ...
    - *Remedy:*  Ensure that a geofile (Raster or shapefile) exists in the specified `CONDITION` folder for the specified cover type (checkbox activated in the GUI). The `Help` button in the GUI provides more information on required geofiles and the [SHArC Working Principles](SHArC-working-principles.md#heprin).
 ...
 - **`ERROR: Could not load boundary Raster: ... .`**
    ...
    - *Remedy:*  Ensure that the spatial boundary was correctly delineated (similar as described in [Project Area Polygon preparation](ProjectMaker.md#pminp2)) and converted to an Integer Raster: On-values have to be Integer 1 and Off-values have to be Integer 0. The sub-*Condition* creation only considers Pixels with On-values (Integer 1).
 ...
 - **`ERROR: Could not load newly created Raster of the project area.`**
    ...
    - *Remedy:*  Ensure that the project was correctly delineated ([Project Area Polygon preparation](ProjectMaker.md#pminp2)).
 ...
 - **`ERROR: Could not perform spatial radius operations [...].`**
    ...
    - *Remedy:*  Ensure that the cover input files and habitat suitability (curve) parameters are properly defined according to [CoverHSI](SHArC.md#hemakecovhsi).
 ...
 - **`ERROR: Could not process information from [...].`**
    ...
    - *Remedy:*  Ensure that the variable (parameters) are properly defined according to [ProjectMaker Quick GUIde](ProjectMaker.md#pmquick) and that the [*SHArC*][6] module contains the required information.
 ...
 - **`ERROR: Could not read parameter type [...] from Fish.xlsx.`**
    ...
    - *Remedy:*
        ...
        + Ensure that a habitat suitability curve is defined in `Fish.xlsx` for the considered hydraulic or cover parameter according to [SHArC Fish section](SHArC.md#hefish).
...
- **`ERROR: Could not read source project (...)`**
 ...
 - **`ERROR: Could not retrieve reach coordinates.`**
    ...
    - *Remedy:*
        + Follow the instructions in [reach preparation within the *ModifyTerrain* module](RiverReaches.md) for correct reach definitions.
	    + If the [*ModifyTerrain*][5] module is externally loaded, ensure the correct definition of features and feature shortnames (see the [alternative run options for the *ModifyTerrain* module](ModifyTerrain.md#mtaltrun)).
 ...
 - **`ERROR: Could not save CSI Raster associated with ...`**
    ...
    - *Remedy:*
        ...
        + Review the input settings according to [SHArC Quick GUIde](SHArC.md#hegui).
 ...
 - **`ERROR: Could not save cover / H HSI [...] Raster ...`**
    ...
    - *Remedy:*
        ...
        + Review the input flow velocity and depth Rasters according to the [Conditions / Input Rasters section](Signposts.md#conditions).
 ...
 - **`ERROR: Could not set arcpy environment (permissions and licenses?).`**
    ...
    - *Remedy:*  Either the wrong *Python* interpreter was used or `arcpy` and/or a *Spatial Analyst* extension are not available. [Read more on setting up the Python environment.](Installation.md#started)
 ...
 - **`ERROR: Could not write value to CELL [...]`**
    ...
    - *Remedy:*  Close all applications that may use `RiverArchitect/SHArC/SHArea/CONDITION_FILI.xlsx`. Detailed information on [*SHArC*][6] workbook outputs are available in the [HHSI input preparation](SHArC.md#hemakehsi) section.
 ...
 - **`ERROR: Cover Raster calculation (check input data).`**
    ...
    - *Remedy:*   Ensure that the input geofiles (Raster or shapefile) are correctly set up according to [CoverHSI](SHArC.md#hemakecovhsi).
 ...
 - **`ERROR: Failed checking *PAR* of *FEATURE*.`**
    ...
    - *Remedy:*
        + Make sure that the `self.parameter_list`s of features ([Code Extension / Extend Features within the *LifespanDesign* module](LifespanDesign-code.md)) has valid entries that also occur in `analysis_call(*args)` (`LifespanDesign/feature_analysis.py`).
	    + Make sure that valid function names exist in `LifespanDesign/cLifespanDesignAnalysis.py` ([Add analysis within the *LifespanDesign* module](LifespanDesign-code.md#addana)).
 ...
 - **`ERROR: Failed to access / Failed to load [...]`**
    ...
    - *Remedy:*
        + Ensure the correct usage of `.site_packages/templates/Fish.xlsx` ([SHArC Fish section](SHArC.md#hefish)) and the completeness of `SHArC/.templates/Q_sharea_template_si.xlsx` and `SHArC/.templates/Q_sharea_template_us.xlsx`. If either template workbook is corrupted or does not exist, re-install missing files.
 ...
 - **`ERROR: Failed to read coordinates from computation_extents.xlsx (return 0).`**
    ...
    - *Remedy:*  Ensure correct setup of `ModifyTerrain/.templates/computation_extents.xlsx` ([reach preparation within the *ModifyTerrain* module](RiverReaches.md)).
 ...
 - **`ERROR: Failed to read maximum depth to water value for [...].`**
    ...
    - *Remedy:*  Ensure the correct setup of `LifespanDesign/.templates/threshold_values.xlsx` ([Modify Threshold Values within the *LifespanDesign* module](LifespanDesign.md#modthresh)). Note that *ModifyTerrain* starts reading depth to water table values column by column, until it meets a non-numeric value.
 ...
 - **`ERROR: Failed to set reach extents -- output is corrupted.`**
    ...
    - *Remedy:*  Follow the instructions in [reach preparation within the *ModifyTerrain* module](RiverReaches.md) for correct reach definitions.
...
- **`ERROR: Flow series analysis failed.`**
 ...
 - **`ERROR: Incoherent data in RAS (Raster comparison).`**
    ...
    - *Remedy:*
        + Manually open the concerned `RAS` Raster and ensure that it complies with the requirements for input Rasters stated in the [Conditions / Input Rasters section](Signposts.md#conditions).
 ...
 - **`ERROR: Input file not available.`**
    ...
    - *Remedy:*
        + Ensure that the file `01_Conditions/CONDITION/input_definitions.inp` exists in the directory `LifespanDesign/.templates/` corresponding to the definitions in the [Input definitions files of the *LifespanDesign* module](Signposts.md#inpfile).
	    + Ensure that the file `mapping.inp` exists in the directory `LifespanDesign/.templates/` corresponding to the definitions in [*LifespanDesign* Output maps](LifespanDesign.md#outmaps).
 ...
 - **`ERROR: Insufficient data. Check Raster consistency and add more flows(?).`**
    ...
    - *Remedy:*
        ...
        + Verify Raster and corresponding lifespan definitions in `01_Conditions/CONDITION/input_definitions.inp` ([Input definitions files of the *LifespanDesign* module](Signposts.md#inpfile)).
 ...
 - **`ERROR: Invalid cell assignment for discharge / Rasters.`**
    ...
    - *Remedy:*  Ensure that the flow duration curve is well defined (see the [HHSI input preparation](SHArC.md#hemakehsi)) and that `RiverArchitect/SHArC/SHArea/CONDITION_FILI.xlsx` is not used by any other application.
    - 
 ...
 - **`ERROR: Invalid feature ID.`**
    ...
    - *Remedy:*
        + Ensure correct definitions in `/LifespanDesign/.templates/threshold_values.xlsx` ([Modify Threshold Values within the *LifespanDesign* module](LifespanDesign.md#modthresh)).
        + Consider replacing corrupted threshold workbooks with the original file.

 - **`ERROR: Invalid file name or data.`**
    - *Cause:*   Error raised by the `save_close_wb(self, *args)` function of the `MakeTable()` or `Write()` class in `.site_packages/riverpy/cMakeTable.py`) or `RiverArchitect/.site_packages/riverpy/cInputOutput.py`), respectively, when it cannot save a workbook (typically `RiverArchitect/SHArC/SHArea/CONDITION_FILI.xlsx` or a copy of the [cost master workbook](ProjectMaker.md#pmcq)).
    - *Remedy:*
        + [*SHArC*][6]: Close all applications that may use `CONDITION_FILI.xlsx` and ensure that its template exists. Detailed information on [*SHArC*][6] workbook outputs are available in the [HHSI input preparation](SHArC.md#hemakehsi).
        + [*ProjectMaker*][7]:  Close all applications that may use the cost master workbook (`REACH_stn_costs_VERSION.xlsx`) and ensure that it exists. Detailed information is available in [ProjectMaker Cost quantity section](ProjectMaker.md#pmcq).
 ...
 - **`ERROR: Invalid interpolation data type (type(Q flowdur) ==  ...)`**
    ...
    - *Remedy:*  Ensure the correct setup of the used flow duration curve in `SHArC/FlowDurationCurves/`. The file structure must correspond to that of the provided template `flow_duration_template.xlsx`. Review the [HHSI input preparation](SHArC.md#hemakehsi) for details.
 ...
 - **`ERROR: Invalid x-y coordinates in extent source [mapping.inp / reaches].`**
    ...
    - *Remedy:*  Ensure that the extent definitions in [`LifespanDesign/.templates/mapping.inp`](Signposts.md#inpfile) and/or [`ModifyTerrain/.templates/computation_extents.xlsx`](RiverReaches.md) are valid numeric values according to the module descriptions.
 ...
 - **`ERROR: Invalid keyword for feature type.`**
    ...
    - *Remedy:*
        + Ensure that code extensions comply with coding conventions and instructions in the [MaxLifespan module code modification possibilities](MaxLifespan.md#actcode). 
...
- **`ERROR: Invalid reference map for [...] (skipping).`**
    ...
    - *Remedy:*  Verify that the project file contains all [default layouts and maps](Mapping.md#lyts) (layout and map names should be identic).
 ...
 - **`ERROR: Lifespan data fetch failed.`**
    ...
    - *Remedy:*
        + Check lifespan/design folder definitions (review [MaxLifespan Quick GUIde](MaxLifespan.md#actquick)).
 ...
 - **`ERROR: Mapping could not assign xy-values. Undefined zoom.`**
    ...
    - *Remedy:*
	    + [*LifespanDesign*][3] or [*MaxLifespan*][4]: Ensure the correct format of [`mapping.inp`](Signposts.md#inpfile) is correct.
        + [*ModifyTerrain*][5]: Ensure correct setup of `ModifyTerrain/.templates/computation_extents.xlsx` ([reach preparation within the *ModifyTerrain* module](RiverReaches.md)).
        + For more settings, review the [Mapping wiki](Mapping.md#modify-map-extents).
 ...
 - **`ERROR: Map layout preparation failed.`**
    ...
    - *Remedy:*
        + [*LifespanDesign*][3]: If a layout was modified ([see default list](Mapping.md#standard-layouts)), ensure that links to databases and datasets are correct.
        + [*MaxLifespan*][4]: Ensure that all relevant layouts ([see default list](Mapping.md#standard-layouts)) are contained in the *ArcGIS* project file (see [mapping descriptions](Mapping.md#about-mapping-with-river-architect)). If needed, add new layouts after code modifications (the [MaxLifespan module code modification possibilities](MaxLifespan.md#actcode)).
	    + <a href="VolumeAssessment">VolumeAssessment</a>: Ensure that all relevant layouts ([see default list](VolumeAssessment.md#vamap)) are contained in the *ArcGIS* project file (see [mapping descriptions](Mapping.md#about-mapping-with-river-architect)).
 ...
 - **`ERROR: Missing (or wrong format of) Raster input definitions.`**
    ...
    - *Remedy:*  Ensure that the file `01_Conditions/CONDITION/input_definitions.inp` exists in the directory `LifespanDesign/.templates/` corresponding to the definitions in the [input definitions files](Signposts.md#inpfile). In case of doubts: Replace `01_Conditions/CONDITION/input_definitions.inp` with the original file and re-apply modifications strictly following the [Conditions / Input Rasters section](Signposts.md#conditions).
 ...
 - **`ERROR: No HSI assigned for parameter type ...`**
    ...
    - *Remedy:*  Ensure that the file `.site_packages/templates/Fish.xlsx` has valid contents according to [SHArC Fish section](SHArC.md#hefish).
...
- **`ERROR: No project/layout available (...).`**
    ...
    - *Remedy:*  Verify that the project file contains all [default layouts](Mapping.md#lyts).
...
- **`ERROR: No reference map found for [...] (skipping).`**
    ...
    - *Remedy:*  Verify that the project file contains all [default layouts](Mapping.md#lyts) (the layout and map names should be identic).
 ...
 - **`ERROR: Scale is not INT. Substituting scale: 2000.`**
    ...
    - *Remedy:*  Ensure that the file `mapping.inp` (in `LifespanDesign/.templates/` or `MaxLifespan /.templates/`) has a correct assignment of the map scale according to the descriptions in [*LifespanDesign* Output maps](LifespanDesign.md#outmaps).
 ...
 - **`ERROR: The provided boundary file (%s) is invalid.`**
    - *Cause:*   Raised the [GetStarted](Signposts.md#getstarted) module's sub-condition creator function (`make_sub_condition` in `RiverArchitect/GetStarted/fSubCondition.py`), when the defined shapefile or Raster contains invalid data.
 ...
 - **`ERROR: Wrong format of lifespan list (.inp)`**
    ...
    - *Remedy:*  Ensure that the file `01_Conditions/CONDITION/input_definitions.inp` (in `LifespanDesign/.templates/`) contains a lifespan list (return periods list) with not more than six comma-separated entries according to the definitions in the [Input definitions files of the *LifespanDesign* module](Signposts.md#inpfile).
 ...
 - **`ExceptionERROR: (arcpy) [...].`**
    ...
    - *Remedy:*
        ...
        + Verify Raster definitions of used parameters (`cParameters.py` and input files `*.inp` according to the [Conditions / Input Rasters section](Signposts.md#conditions)).
 ...
 - **`ExceptionERROR: Cannot find package files [...].`**
    ...
    - *Remedy:*  Check the installation of the package and its file structure according to the [Requirements stated in the *Introduction and Installation*](Installation.md#req).
 ...
 - **`ExceptionERROR: Missing fundamental packages (required: ...).`**
    ...
    - *Remedy:*  Check installation of required packages and code structure files according to the [Requirements stated in the *Introduction and Installation*](Installation.md#req).
...
- **`OSError: ....`**
    ...
    - *Remedy:*  Check the correct installation of *River Architect* and input files (refer to the <a href="Installation">Installation</a>  and <a href="Signposts">Signposts</a> pages and use the [correct Python environment](Installation.md#install-and-get-started)). Make sure that no other program uses files required by *River Architect*.
...
================
 ...
 - **`WARNING: Cannot identify reaches.`**
    ...
    - *Remedy:*  Ensure that `ModifyTerrain/.templates/computation_extents.xlsx` does not contain more than eight reaches (i.e., only cells `B6:C13` contain reach names and identifiers, also see [reach preparation within the *ModifyTerrain* module](RiverReaches.md)).
 ...
 - **`WARNING: Cannot zoom to defined extent.`**
    ...
    - *Remedy:*
	    + All modules: Ensure that the [`mapping.inp`](Signposts.md#inpfile) and the [`reach definitions`](RiverReaches.md) files contain valid data.
...
- **`WARNING: Could not clear/remove .cache.`**
 ...
 - **`WARNING: Could not clean up PDF map temp_pages.`**
    - *Cause:*   The `make_pdf_maps(self, map_name, *args, **kwargs)` functions of the `Mapper()` class (`.site_packages/riverpy/cMapper.py`) creates single `PDF`s of every map image (extents defined in [mapping.inp](Signposts.md#inpfile) or the [reach coordinates workbook](RiverReaches.md)). These single-page `PDF`s are finally combined into one `PDF` map assembly and the single-page `PDF`s are deleted afterward. 
...
- **`WARNING: Could not find Lifespan Raster (...).`**
 ...
 - **`WARNING: Could not read project area extents.`**
    ...
    - *Remedy:*  Ensure that the `ProjectArea.shp` shapefile is correctly created (in particular the *Attributes Table*), according to [Project Area Polygon preparation](ProjectMaker.md#pminp2).
 ...
 - **`WARNING: Design map - Could not assign frequency threshold. [...]`**
    - *Cause:*   Design maps, such as stable grain size, refer to hydraulic data related to a defined return period. If`design_...` functions ) `LifespanDesign/cLifespanDesignAnalysis.py`) cannot identify a particular`threshold_freq` value,`design_...` functions automatically try to use hydraulic data related to the first entry of `lifespans* (`Return periods` entry in `01_Conditions/CONDITION/input_definitions.inp`, see the [Input definitions files of the *LifespanDesign* module](Signposts.md#inpfile)).
    - *Remedy:*
        + Assign a float value to the concerned feature in the `Mobility frequency threshold` row of the `LifespanDesign/.templates/threshold_values.xlsx` spreadsheet (see also [Modify Threshold Values within the *LifespanDesign* module](LifespanDesign.md#modthresh)).
	    + Make sure that the defined defined `Mobility frequency threshold* float is consistent with the defined `Return periods` in `01_Conditions/CONDITION/input_definitions.inp` (see the [Input definitions files of the *LifespanDesign* module](Signposts.md#inpfile)).
 ...
 - **`WARNING: Failed to align input raster [...].`**
    ...
    - *Remedy:*  Verify if the stated Raster contains invalid values (exists). Complex filenames that do not comply with the *River Architect* [filename convention](Signposts.md#terms) may also cause this error. Rename all files with the `Tools/rename_files.py` script (see inline code comments for instructions) to adapt all files to the *River Architect* [filename convention](Signposts.md#terms) and re-run `Align Rasters` (if required).
 ...
 - **`WARNING: Invalid curve data (Fish.xlsx): PAR= ... , HSI= ... .`**
    ...
    - *Remedy:* 
        + Ensure that *Physical Habitats* in `Fish.xlsx` are correctly defined according to the [definitions](SHArC.md#hefish).
 ...
 - **`WARNING: Invalid feature names for column headers.`**
    ...
    - *Remedy:* 
        + Ensure that `ModifyTerrain/.templates/computation_extents.xlsx` contains valid reach descriptions ([reach preparation within the *ModifyTerrain* module](RiverReaches.md)).
 ...
 - **`WARNING: Invalid alignment Raster name ([...]).`**
    ...
    - *Remedy:*  Complex filenames that do not comply with the *River Architect* [filename convention](Signposts.md#nameconvention) may also cause this error. Rename all files with the `Tools/rename_files.py` script (see inline code comments for instructions) to adapt all Raster filenames to the *River Architect* [filename convention](Signposts.md#terms) and re-run `Align Rasters` (if required).
 ...
 - **`WARNING: PlantExisting.shp is corrupted or non-existent.`**
    ...
    - *Remedy:*  Ensure that existing plants (`ProjectMaker/ProjectName/Geodata/Shapefiles/PlantExisting.shp`) is correctly set up [according to the descriptions in the wiki](ProjectMaker.md#pminp31). However, this raster is not absolutely required.
 ...
 - **`WARNING: Substituting user-defined crit. lifespan (...) with ....`**
    ...
    - *Remedy:*  Ensure to only use lifespans defined for the provided *Condition* ([cf. definition of flow duration periods](Signposts.md#ana-flows)).
